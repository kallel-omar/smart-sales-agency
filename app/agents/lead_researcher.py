    ...
    async def run(self, lead: Lead) -> dict:
        ...
        if self.context.settings.llm_mode == "demo":
            ...
            summary = (
                ...
                f"Available signals: {', '.join(available_signals) if available_signals else 'basic lead data only'}. "
                "This is a demo brief and contains no external web research."
            )
            evidence = [
                {"type": "lead_input", "field": "company_name", "value": lead.company_name},
                {"type": "lead_input", "field": "website", "value": lead.website},
            ]
        else:
            system = (
