...
class QualificationAgent:
    ...
    async def run(self, lead: Lead, research: dict) -> QualificationResult:
        score = 10
        reasons: list[str] = []
        ...
        if research.get("opportunities"):
            score += min(20, len(research["opportunities"]) * 10)
            reasons.append("The research brief identified relevant opportunities")

        score = min(score, 100)
        qualified = score >= self.threshold
        self.context.repository.update_lead_score(lead, score, qualified)
        return QualificationResult(score=score, qualified=qualified, reasons=reasons)
